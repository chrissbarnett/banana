...
define([
    ...
    function (angular, app, _, kbn, moment, uiBootstrap) {
        'use strict';

        var module = angular.module('kibana.panels.posterTable', []);
        app.useModule(module);
        module.controller('posterTable', function ($rootScope, $scope, fields, querySrv, dashboard, filterSrv, $filter,
                                                   $modal, $timeout) {
            $scope.panelMeta = {
                modals: [
                    {
                ...
                editorTabs: [
                    {
                        title: 'Fields',
                        src: 'app/panels/posterTable/fields.html'
                    },
                    {
                        title: 'Paging',
                        src: 'app/panels/posterTable/pagination.html'
                    },
                    {
                        title: 'Queries',
            ...
            var _d = {
                ...
                imgFieldHeight: 'auto', // height of <img> (if enabled)
                show_queries: true,
                dateFormat: 'YYYY',
                aliases: []
            };
            ...
            $scope.init = function () {
                $scope.Math = Math;
                // Solr
                $scope.sjs = $scope.sjs || sjsResource(dashboard.get_data_core()); // jshint ignore: line
            ...
            angular.element(window).bind('resize', function () {
                //if ($scope.gallery.show){
                //reset poster size
                $scope.setPreviewSize();
                //}

            });


            $scope.setPreviewSize = function () {
                var h = $(window).height();
                var w = $(window).width();
                var ch = $(".poster-citation").height() || 150;
                var css = {
                    'max-width': w - 150,
                    'max-height': h - ch - 100
                };
                $(".poster-image img").css(css);
            };
            ...
            $scope.poster = {};
            $scope.set_preview_poster = function (index) {
                //console.log(index);
                var row = $scope.data[index];
                if (_.isUndefined(row)) {
                    throw new Error("No data for this index.");
                }
                $scope.poster = {
                    title: row.FullPosterTitle,
                    date: row.Semester + " " + row.Year,
                    author: row.StudentName.join(),
                    preview: get_fullsize_preview(row.WebLinkPoster),
                    link: row.WebLinkPoster,
                    index: index
                };
            };

            $scope.hasNext = function (index) {
                var next = $scope.data.length > (index || false);
                if (!next) {
                    next = $scope.hasNextPage();
                }
                return next;
            };
            ...
            $scope.hasPrev = function (index) {
                var prev = index > 0;
                if (!prev) {
                    prev = $scope.hasPrevPage()
                }
                return prev;
            };


            $scope.previewNext = function (currIndex) {
                if ($scope.hasNext(currIndex)) {
                    if (currIndex > $scope.data.length - 2) {
                        $scope.nextPage().then(function () {
                            $scope.set_preview_poster(0);

                        });

                    } else {
                        $scope.set_preview_poster(currIndex + 1);
                    }
                }
            };

            $scope.previewPrev = function (currIndex) {
                if ($scope.hasPrev(currIndex)) {
                    if (currIndex === 0) {
                        $scope.prevPage().then(function () {
                            $scope.set_preview_poster($scope.data.length - 1);
                        });
                    } else {
                        $scope.set_preview_poster(currIndex - 1);
                    }
            ...
            $scope.$on('modal.hide', function () {
                dashboard.stopScroll = false;
            });
            $scope.$on('modal.show', function () {
                dashboard.stopScroll = true;
            });

            $scope.gallery = $modal({
                scope: $scope,
                templateUrl: 'app/panels/posterTable/gallery.tpl.html',
                show: false
            });

            $scope.showModal = function () {
                $scope.gallery.$promise.then($scope.gallery.show).then($scope.setPreviewSize);
            };

            $scope.open_gallery = function (index) {

                $scope.set_preview_poster(index);
                /*row.kibana.details = row.kibana.details ? false : true;
                 row.kibana.view = row.kibana.view || 'posterTable'*/
            ...
            $scope.nextPage = function () {
                ...
                if (page.current < page.total) {
                    return $scope.page(page.current + 1);
                }
            };
            ...
            $scope.getCurrentPage = function () {
                var totalpages = Math.ceil($scope.panel.hits / $scope.panel.size);
                var currentPage = $scope.panel.offset / $scope.panel.size;
                var page = {
                    current: currentPage,
                    total: totalpages
                };
            ...
            $scope.get_data = function (segment, query_id) {
                ...
                var request = $scope.get_request(_segment).doSearch();
            ...
            $scope.get_data_promise = function (segment, query_id) {
            ...
            var get_filename_from_path = function (str) {
                return str.split('\\').pop().split('/').pop();
            };

            var get_fullsize_preview = function (val) {
                val = get_filename_from_path(val);
                val = val.replace(".pdf", ".png");
                val = "img/posters/thumbnails/1500/" + val;
                return val;
            };
            ...
            $scope.get_field_value = function (field, value) {
                ...
                var text = "";
                if (field === "WebLinkPoster") {
                    value = get_filename_from_path(value);
                    value = value.replace(".pdf", ".png");
                    value = "img/posters/thumbnails/300/" + value;
                    text = $filter('tableDisplayImageField')(value, $scope.panel.imageFieldWidth, $scope.panel.imageFieldHeight);

                } else if (field_type === "tdate") {
        ...
        module.filter('posterThumbnail', function () {
