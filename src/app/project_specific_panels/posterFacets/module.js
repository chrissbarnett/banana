...
define([
    ...
    function (angular, app, _, kbn) {
        ...
        var module = angular.module('kibana.panels.posterFacets', []);
        app.useModule(module);

        module.controller('posterFacets', function ($rootScope, $scope, fields, querySrv, dashboard, filterSrv) {
            $scope.panelMeta = {
                modals: [{
                    description: "Inspect",
                    icon: "icon-info-sign",
            ...
            $scope.showOnlyWinners = false;
            $scope.toggle_winner = function () {
                $scope.showOnlyWinners = !$scope.showOnlyWinners;
                ...
                if ($scope.showOnlyWinners) {
                    filterSrv.set(filter);

                } else {
                    filterSrv.removeByTypeAndField(filter.type, filter.field);
                }
                dashboard.refresh();
            ...
            $scope.get_data = function (segment, query_id) {
                ...
                var wt_json = '&wt=json';
                var facet = '&facet=true&facet.method=fc';
                var rows = '&rows=0';
                var facet_fields = '';
                ...
                results.then(function (results) {
                    ...
                    if ($scope.query_id === query_id) {
                        ...
                        var exclude = {
                            "field": "Method",
                            "facets": ["Spatial Analysis", "Geostatistical Analysis", "Spatial Interpolation", "Needs Analysis"]
                        };
                        _.each(facet_results, function (counts, field) {
                            facet_data[field] = [];
                            for (var i = 0; i < counts.length; i += 2) {
                                //TODO: temporary... exclude some values
                                if (field == exclude.field) {
                                    if (exclude.facets.indexOf(counts[i]) > -1) {
                                        continue;
                                    }
                                }
